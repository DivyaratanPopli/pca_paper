...
sim_func <- function(sp, Ne, gentime, st_array, mgen_array, madm_array, mgen, mprop,
  ...
  gf <- list(
    gene_flow(from = pop5, to = pop6, start = mgenF-gentime, end = mgenF-(2*gentime), rate = 1-madmF),
    gene_flow(from = pop4, to = pop7, start = mgenG-gentime, end = mgenG-(2*gentime), rate = 1-madmG),
    gene_flow(from = pop3, to = pop8, start = mgenH-gentime, end = mgenH-(2*gentime), rate = 1-madmH),
    gene_flow(from = pop4, to = pop9, start = mgenI-gentime, end = mgenI-(2*gentime), rate = 1-madmI),
    gene_flow(from = pop2, to = pop10, start = mgenJ-gentime, end = mgenJ-(2*gentime), rate = 1-madmJ),
...
sim_func(sp=snakemake@wildcards[["sp"]], Ne=snakemake@wildcards[["Ne"]], gentime= snakemake@params[["gentime"]],
  st_array=snakemake@params[["st_array"]],
  mgen_array=snakemake@params[["mgen_array"]], madm_array=snakemake@params[["madm_array"]],
  mgen=snakemake@params[["mgen"]], mprop=snakemake@wildcards[["mu"]],
  mutation_rate=snakemake@params[["mutation_rate"]], recombination_rate=snakemake@params[["recombination_rate"]],
  slength=snakemake@params[["length"]],
  s1=snakemake@wildcards[["nind"]], genfile=snakemake@output[["genfile"]],
  outf2=snakemake@output[["outf2"]], outfile=snakemake@output[["outfile"]])

#sim_func(sp=1000, Ne=1000, gentime=30, st_array=c(1,2,5,5.5), mgen_array=c(1.5, 0.4, 0.1, 2.4, 0.2), madm_array=c(0.8, 0.5, 0.4, 0.6, 0.6), nanc_array=c(0.8, 0.3, 0.2, 1), mgen=0.5, mprop=0,
#         mutation_rate=1e-8, recombination_rate=1e-8, slength=1e8, s1=100,
#         genfile="gen.csv", outfile="f.csv", outf2="f2.csv")
