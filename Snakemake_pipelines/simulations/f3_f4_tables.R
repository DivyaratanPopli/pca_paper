...
plot_f2s <- function(f_avg, f_std, ftrue, f3out, f4out){
  ...
  for (i in seq(1,length(f_avg))){
    ...
    df=df[df$Method!="admixtools2Norm_scale12",]
  ...
  write.table(df1, file = f3out, row.names=FALSE, col.names=TRUE, sep=',')
  write.table(df2, file = f4out, row.names=FALSE, col.names=TRUE, sep=',')
