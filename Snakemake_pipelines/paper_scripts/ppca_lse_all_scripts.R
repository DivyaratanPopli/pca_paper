...
fstats_pca <- function(dataf, outfile, outfile_s, outfile_d, method, npcs, sigmaf){
  
  data1=read.csv(file=dataf, header=F, sep=',')
  is.na(data1) <- data1 == "9"
  data1=data1/2
  mu = rowMeans(data1,na.rm=TRUE)
  data = data1 - mu
  ...
  if (method=='pca'){
    ...
    xxxx=lse_miss(data=as.matrix(data1), n_pcs=npcs, normalized=T)
    pc=t(xxxx$pcs)
  }
...
fstats_pca(dataf=dataf, outfile="outf", outfile_s="outfs", outfile_d="outfd", method="lse_miss", npcs=100, sigmaf="sigma")
